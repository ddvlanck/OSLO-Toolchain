...
describe('Associations with subclasses', () => {
  const configuration = new Configuration(
    // eslint-disable-next-line max-len
    'https://github.com/Informatievlaanderen/OSLOthema-toolchainTestbed/blob/master/testAssociatiesMijnAfdaalDomein.EAP?raw=true',
    'MijnAfdaalDomeinAP',
    'subclasses.report.test.jsonld',
    'ApplicationProfile',
  );
  ...
  it('should handle an EAP file containing subclasses', async () => {
    const converter = new EaConverter(configuration, outputHandler);
    await converter.convert();

    const target = await readFile('test/outputFiles/AssociationsWithSubclasses.report.jsonld');
